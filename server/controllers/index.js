...
// Creates a new dog based on user input
// Requires a name, breed, and age
const createDog = async (req, res) => {
  if (!req.body.name || !req.body.breed || !req.body.age) {
    return res.status(400).json({ error: 'name, breed, and age are all required' });
  }

  // Create the dog if all requirements are met
  const dogData = {
    name: req.body.name,
  ...
  // Attempt to save dog to database, return internal server error if fails
  try {
    await newDog.save();
    return res.status(201).json({
      name: newDog.name,
...
// Finds a dog in the MongoDB and increases its age by 1 year
const addDogAge = async (req, res) => {
  if (!req.body.name) {
    return res.status(400).json({ error: 'a name is required to perform the search' });
  }

  // Check if the dog exists, store it in doc if so
  let doc;
  try {
    doc = await Dog.findOne({ name: req.body.name }).exec();
  } catch (err) {
    console.log(err);
    return res.status(500).json({ error: 'Something went wrong when searching for dog.' });
  }
  ...
  // Attempt to update dog's age by 1 year
  const newAge = doc.age + 1;
  try {
    await Dog.updateOne({ name: doc.name }, { age: newAge }).exec();
  } catch (err) {
    console.log(err);
